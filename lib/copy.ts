...
const vi: Strings = {
  ...
  hero: {
    hi: "Xin chào!",
    name: "Mình là Nguyễn Xuân Nhân",
    /* Empty: a pronunciation guide is pointless for native speakers. */
    pron: "",
    roles: [
      "Sinh viên năm cuối @Gettysburg College",
      "Prev. SWE Intern @KBSecuritiesVietnam",
      "Prev. SWE Intern @FinbudAI",
      "Prev. SWE Intern @eSmartSolutionsAgency",
      "Prev. FE Intern @FPTIS",
    ...
    schools: [
      {
        ...
        blurb:
          "Trường chuyên ở TP.HCM, ba năm cuối ở nhà trước khi sang Mỹ.",
        photo: "ẢNH LỚP HỌC",
      },
      {
        id: "gettysburg",
        ...
        name: "GETTYSBURG COLLEGE · 2022–2027",
        blurb:
          "B.S. Computer Science, minor Business. Trường khai phóng nên mình vừa code vừa viết và model được.",
        photo: "ẢNH CAMPUS",
      },
    ],
    ms: [
      { year: "2022", text: "Từ Sài Gòn sang. Lớp CS đầu tiên, mùa tuyết đầu tiên." },
      {
        year: "2024",
        text: "Kỳ intern kỹ thuật đầu tiên. Học được là pipeline hay sập lúc 3 giờ sáng.",
      },
      { year: "2025", text: "SWE Intern mảng covered warrants tại KB Securities." },
      { year: "2026 →", text: "Đang tìm vị trí summer 2027." },
    ],
  },
  work: {
    label: "EXPERIENCE",
    jobs: [
      {
        role: "SWE Intern",
        blurb:
          "Hai sản phẩm chạy trên cùng một market-data spine: live terminal cho desk chứng quyền, và CW Portal public của KBSV. Phần khó không nằm ở công thức pricing, mà ở chỗ giữ cho luồng quote đủ đúng để trader tin con số trên màn hình.",
        bullets: [
          "Đẩy differential patch qua socket thay vì full snapshot. Client phải tự reconcile state — đó là cái giá để bandwidth chỉ còn một phần nhỏ.",
          "Terminal của desk và portal public nằm trên hai VM riêng, nên traffic public tăng đột biến không bao giờ giành tài nguyên với feed giao dịch.",
          "Data layer là pipeline riêng chứ không nằm trong code app, nên dedup, normalise và adjusted price chỉ xử lý một lần, cả hai sản phẩm đọc cùng một lịch sử.",
        ],
      },
      {
        role: "SWE Intern",
        blurb:
          "Nền tảng phân tích tài chính trả lời câu hỏi dựa trên một corpus tài liệu. Hoá ra gần hết thời gian là ngồi chờ I/O chứ không phải chờ model, và điều đó đổi hẳn thứ đáng tối ưu — vấn đề nằm ở plumbing, không phải ở prompt.",
        bullets: [
          "Latency giảm nhờ async request flow và cache retrieval chứ không phải nhờ đổi sang model lớn hơn — thời gian trôi ở chỗ chờ I/O, không phải ở inference.",
          "Nén prompt trước khi gửi, rồi kiểm lại xem chất lượng câu trả lời có giữ được không: rẻ hơn và nhanh hơn chỉ có nghĩa nếu output chịu được.",
          "Container hoá backend và chạy full test suite mỗi lần push, để khoảng cách giữa máy mình và QA không còn là chỗ cho bug ẩn náu.",
        ],
      },
      {
        role: "SWE Intern",
        blurb:
          "Content platform biến một brief thành nhiều bản draft cho từng kênh. Giao trọn cho một model thì output nhạt và na ná nhau, nên pipeline được chẻ thành các stage chấm điểm và sửa riêng được.",
        bullets: [
          "Route theo task thay vì dùng chung một model: model rẻ lo extraction và phân loại, model mạnh lo phần viết thực sự.",
          "Biến chất lượng thành một vòng lặp chứ không phải một câu prompt — mỗi draft được score theo tín hiệu SEO bên ngoài rồi prompt lại đến khi đạt ngưỡng.",
          "Gom shared state của dashboard về một chỗ để sửa ở màn này không làm màn kia hiện dữ liệu cũ — đó là loại bug user báo nhiều nhất.",
        ],
      },
      {
        role: "Frontend Engineer Intern",
        blurb:
          "Kỳ intern đầu tiên: hệ thống procurement B2B cho Rạng Đông, user là nhân viên mua hàng chứ không phải kỹ sư. Lỗi đắt nhất ở đó không phải crash, mà là một con số gõ sai lặng lẽ chạy vào ngân sách rồi mấy tháng sau mới lòi ra.",
        bullets: [
          "Đưa validation vào ngay bước tạo và sửa kế hoạch, vì bắt được con số sai lúc nhập đáng giá hơn mọi báo cáo về sau.",
          "Với hơn 200.000 kế hoạch trong hệ thống, navigation chính là tính năng — tìm được đúng cái cần quan trọng hơn việc nó render nhanh cỡ nào.",
          "Tự làm phân tích ngân sách hàng tuần cho 16 hạng mục IT, nhờ vậy mới biết UI thực sự cần đưa con số nào ra trước mắt người dùng.",
        ],
      },
    ],
  },
    ...
    items: [
      {
        ...
        thumb: "RESEARCH TERMINAL\nquotes → greeks → llm",
        blurb:
          "Research terminal cho hơn 100 chứng quyền Việt Nam, stream market data FiinQuant qua FastAPI và WebSockets.",
        bullets: [
          "Pricing Black-Scholes-Merton, implied và historical volatility, các Greeks",
          "LLM assistant gọi tool, bám theo quote trực tiếp và lịch sử giá",
          "Hiện tool trace và guardrail không tư vấn đầu tư",
        ],
      },
      {
        kicker: "PROJECT 02",
        ...
        thumb: "cw.kbsec.com.vn\nnext.js · fastapi · kafka",
        blurb:
          "Cổng chứng quyền mình build và đưa lên production ở KB Securities, đang phục vụ trader và nhà đầu tư cá nhân.",
        bullets: [
          "Kiến trúc 2 VM, tách Next.js/Nginx khỏi các service container hoá",
          "16 REST endpoint kèm unit test và integration test bằng pytest",
          "Kafka và PostgreSQL sau một tầng market data cache bằng Redis",
        ],
      },
      {
        kicker: "PROJECT 03",
        ...
        thumb: "ITCH FEED HANDLER\nc++20 · l2 order book",
        blurb:
          "Feed handler Nasdaq TotalView-ITCH 5.0 viết bằng C++20, dựng lại aggregated L2 order book trực tiếp từ luồng event thô, đo trên trọn một phiên 11,25 GB.",
        bullets: [
          "368 triệu message dựng thành 8.695 order book theo mã, verify bằng 24 kịch bản correctness và UBSan",
          "Profiling dẫn tới việc đổi layout symbol interning: nhanh thêm 12,3% trên synthetic, không claim gì trên phiên thật",
          "Engineering log giữ lại cả những tối ưu không ăn thua, kèm số liệu",
        ],
      },
    ],
  },
  mac: {
    ...
    hintTerminal: "cd vào một project rồi run.",
    notDeployed: "not deployed",
    unknown: "command not found",
    selected: "selected",
    noSelection: "chưa chọn project - thử: cd cw",
  ...
  contact: {
    ...
    email: "EMAIL",
    msg: "NỘI DUNG",
    namePh: "ví dụ: Nguyễn Xuân Nhân",
    emailPh: "ví dụ: xuannhan.nguyen@gmail.com",
    msgPh:
      "Chào Nhân, mình muốn hẹn một buổi meeting... (con blob đang ăn mừng rồi)",
    btn: "GỬI",
    sent: "ĐÃ GỬI ✓ CẢM ƠN!",
